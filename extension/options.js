...
(function () {
  ...
  copyLogBtn.addEventListener('click', function() {
    ...
    chrome.runtime.sendMessage({ type: 'GET_LOG' }, function(response) {
      if (chrome.runtime.lastError || !response) { statusMsg.textContent = 'Error fetching log.'; return; }
      navigator.clipboard.writeText(response.log).then(function() {
        ...
        setTimeout(function() { copyLogBtn.textContent = 'Copy Debug Log'; copyLogBtn.classList.remove('success'); statusMsg.textContent = ''; }, 2500);
      }).catch(function() { statusMsg.textContent = 'Clipboard unavailable.'; });
    });
  });
})();
